...
experiment_alabama <- alabama_file3[5:43]
```


```{r}
for (i in 1:length(experiment_alabama)) {
  if(str_detect(experiment_alabama[i], "01-") == FALSE & !is.na(experiment_alabama[i])) {
    experiment_alabama[i] <- paste(experiment_alabama[i], experiment_alabama[i+1], sep = "")
    experiment_alabama[i+1] <- NA
  }
}
experiment_alabama <- experiment_alabama %>%
  na.omit()
```


```{r}
alabama_index_company_name <- vector(mode="character", length=11)
alabama_index_company_code <- vector(mode="character", length=11)

tracking_number <- 1

for (i in 1:length(experiment_alabama)) {
  string <- experiment_alabama[i]
  m <- gregexpr('[0-9]+',string)
  something <- regmatches(string, m)
  something <- c(something[[1]])
  alabama_index_company_code[tracking_number] <- paste(something, collapse = '-')
  vector_string <- str_split(string, "01")
  vector_string <- vector_string[[1]]
  alabama_index_company_name[tracking_number] <- vector_string[1]
  tracking_number <- tracking_number + 1
}

table_alabama_index <- data.frame(alabama_index_company_name, alabama_index_company_code)
